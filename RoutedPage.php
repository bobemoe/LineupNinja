...
abstract class RoutedPage{
  ...
  */
  ...
  function detectPost($posts){
    ...
    */
    if (preg_match($this->route_regexp,$wp->request,$matches) ){
      //remove the post info
      add_filter( 'genesis_post_info', [$this,'getHeader'] );
      add_filter( 'genesis_post_meta', [$this,'getFooter'] );

      //Add the fake post
      $posts=NULL;
      $posts[]=$this->createPost($wp->request,$matches);
  ...
  */
  public function createPost($url,$matches)
  {
    ...
    */
    ...
    // let the extending classes set their own post data now
    $post=$this->setupPost($post,$matches);

    return($post);
  }

  public abstract function setupPost($post,$matches);
